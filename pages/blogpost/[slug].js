...
export default function BlogPostPage({ blogs, comments, replyComments })
{
  ...
  const replaceCodeBlocks = (content) =>
  {
    ...
    codeBlocks.forEach((block, index) =>
    {
      ...
      const language = languageClass ? languageClass.replace('language-', '') : 'jsx';
  ...
  const PostReplyComment = async (id) =>
  {
    if (session) {
      ...
      if (repliedComments.includes(id)) {
        toast('You have already replied to this comment', { type: 'info', autoClose: 3000 });
        return;
      }
  ...
  const seo = {
    title: BlogPost ? BlogPost.title + ' | CodeWithRafay' : 'Web Development Agency | CodeWithRafay',
            ...
            <div className="container max-w-full md:max-w-screen-lg bg-white mx-auto my-8 p-4 md:p-10 rounded-lg text-center shadow-lg dark:bg-gray-800">
              ...
              <div className="flex mt-2 gap-2 md:text-sm items-center justify-center text-slate-500 after-title">
                <div className="flex items-center justify-center gap-1">
                  ...
                  <h5 className=" ml-1 text-black dark:text-white">Abdul Rafay</h5>
              ...
              </div>
              {
                BlogPost.BlogImage ? (
                  <div className='flex justify-center my-3'>
                    <Image width={550} height={100} src={BlogPost.BlogImage} alt="image cwr blog" priority={true} />
                  </div>
                ) : ''
              }
              <div
                className="blog-content text-left mt-3 dark:text-gray-200"
                dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(replaceCodeBlocks(BlogPost.BlogContent)) }}
              />
            </div>
          </div>
        )}
        ...
        <div className="container max-w-full md:max-w-screen-lg mx-auto p-4 mt-2 bg-white rounded-lg shadow-lg dark:bg-gray-800">
          <form onSubmit={PostComment}>
            ...
            <button
              ...
              disabled={BlogPostComments.some(item => item.username === (session?.email ? session.email.split('@')[0] : ''))}
            >
              Add Comment
            </button>
          </form>
